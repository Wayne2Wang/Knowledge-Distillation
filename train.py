...
def parse_arg():
    ...
    # Last command line option: --bs 4 --hs 100 100 100 --model custom_mlp
    """
    parser = argparse.ArgumentParser(description='Arugments for fitting the feedforward model')
    parser.add_argument('--model', default='mlp', help='Specify a model to train (resnet / mlp / custom_mlp)')
...
def main():
    ...
    if dataset == 'ImageNet1k':    
        trainset, valset = ImageNet(root='data/ImageNet1k/',flat=(False if modeltype != 'mlp' else True),verbose=verbose, img_generator=False)
        ...
        trainset, valset = ImageNet(root='D:\Research\Dataset\ImageNet64\Imagenet64_batch',flat=True, verbose=verbose, img_generator=True, imgsize=64) # batch files
        output_size = 1000 # number of distinct labels
        input_size = (3*64*64,)
    else:
